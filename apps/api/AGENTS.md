...
- **Every row travels through TypeORM** ([ADR 0005](../../docs/adr/0005-the-api-reaches-its-data-through-typeorm.md)): one long-lived `DataSource` on the session pooler, `synchronize: false`, migrations generated from the entities. The service client in `_config/supabase.config.ts` never touches data — it serves `auth.admin.deleteUser` and Theme image upload signing, metadata checks and cleanup ([ADR 0007](../../docs/adr/0007-storage-bytes-never-transit-the-api.md)), nothing else. There is still no per-request user-authed client, so owner scoping is explicit owner filters in the repository.
...
src/
  ...
  health/         # GET /health
  _config/        # ConfigModule and its providers: ENV (zod-validated, parsed once at boot), SUPABASE (auth admin and storage operations, never data), JWKS
