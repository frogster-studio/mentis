...
describe("verifyCredentials", () => {
  it("accepts an allowlisted email with the correct password", () => {
    expect(
      verifyCredentials({ email: "alice@example.com", password: env.sharedPassword }, env),
    ).toBe(true);
  });
  ...
  it("rejects an email that is not on the allowlist", () => {
    expect(
      verifyCredentials({ email: "mallory@example.com", password: env.sharedPassword }, env),
    ).toBe(false);
  });

  it("rejects a wrong password for an allowlisted email", () => {
    expect(verifyCredentials({ email: "alice@example.com", password: "wrong" }, env)).toBe(false);
  });
  ...
  it("rejects everything when the shared password env var is missing or empty", () => {
    for (const sharedPassword of [undefined, ""]) {
      expect(
        verifyCredentials({ email: "alice@example.com", password: "" }, { ...env, sharedPassword }),
      ).toBe(false);
    }
  });
