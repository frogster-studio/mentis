...
export function cardImagePublicUrl(client: SupabaseClient, path: string): string {
  return client.storage.from(CARD_IMAGES_BUCKET).getPublicUrl(path).data.publicUrl;
}

// Removing an Image from a Card must also drop its object, or the bucket
// accumulates orphans no row references.
export async function removeCardImages(client: SupabaseClient, paths: string[]): Promise<void> {
