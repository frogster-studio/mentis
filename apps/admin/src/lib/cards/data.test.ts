...
function createFakeSupabase(options?: {
  ...
  function sortRows(subset: Record<string, unknown>[], column: string, ascending: boolean) {
  ...
  function createReadQuery() {
    ...
    const builder = {
      ...
      contains(column: string, values: unknown[]) {
        filters.push((row) => values.every((value) => (row[column] as unknown[]).includes(value)));
        return builder;
      },
      ...
      then(resolve: (result: { data: Row[]; count: number; error: null }) => void) {
        let subset = matching();
        if (ordering) {
                ...
                select() {
                  return {
                    async single() {
                      ...
                      const contentColumns = ["type", "title", "tags", "payload", "images"];
              ...
              return {
                select(_columns?: string) {
                  return {
                    // biome-ignore lint/suspicious/noThenProperty: the real Supabase query builder is a thenable; the fake must be too.
                    then(
                      resolve: (result: { data: Record<string, unknown>[]; error: null }) => void,
                    ) {
...
describe("listCards with a Title search", () => {
  it("matches partial Titles case-insensitively", async () => {
    ...
    expect(cards.map((card) => card.title)).toEqual(["La prise de la Bastille"]);
  });
...
describe("listCards pagination", () => {
  ...
  it("slices pages server-side, newest first, and reports the totals", async () => {
    ...
    expect(firstPage.cards.map((card) => card.title)).toEqual(["Card 5", "Card 4"]);
...
describe("getCard", () => {
  it("loads a Card by id and returns null for an unknown id", async () => {
    ...
    expect(await getCard(client, "ffffffff-ffff-ffff-ffff-ffffffffffff")).toBeNull();
  });
});
...
describe("updateCard", () => {
  it("round-trips an edit and bumps the updated timestamp", async () => {
    ...
    const updated = await updateCard(client, created.id, anecdote("Après", "nouveau"));
  ...
  it("surfaces an update failure as an error", async () => {
    ...
    await expect(updateCard(client, "00000000-0000-0000-0000-000000000001", card)).rejects.toThrow(
      "Failed to update Card: boom",
    );
  });
});
...
describe("setCardPostedOn", () => {
  it("marks and unmarks Socials, one at a time", async () => {
    ...
    const unmarked = await setCardPostedOn(client, created.id, "linkedin", false);
  ...
  it("throws for an unknown Card", async () => {
    ...
    await expect(
      setCardPostedOn(client, "ffffffff-ffff-ffff-ffff-ffffffffffff", "x", true),
...
describe("Quiz Cards", () => {
  ...
  it("round-trips a Quiz update with a changed correct Choice", async () => {
    ...
    const created = await insertCard(client, quiz("Bastille", quizPayload(1, "À corriger.")));
    const newPayload = quizPayload(0, "Le 14 juillet 1789.");

    const updated = await updateCard(client, created.id, quiz("Bastille", newPayload));
...
describe("Riddle Cards", () => {
  it("round-trips a create with Bonus Info and an update dropping it", async () => {
    ...
    const updated = await updateCard(client, created.id, riddle("Sphinx", withoutBonus));
...
describe("Did You Know Cards", () => {
  it("round-trips a create and update under its own type value", async () => {
    ...
    const updated = await updateCard(
      client,
      created.id,
      didYouKnow("Tour Eiffel", "La dilatation la fait grandir de 15 cm l'été."),
    );
...
describe("Card Images", () => {
  ...
  it("round-trips the Image list order through a reorder", async () => {
    ...
    const created = await insertCard(client, anecdoteWithImages("Illustrée", threeImages));
    expect(created.images.map((image) => image.path)).toEqual(["a.webp", "b.webp", "c.webp"]);
    ...
    expect(reloaded?.images.map((image) => image.path)).toEqual(["c.webp", "a.webp", "b.webp"]);
  ...
  it("persists Caption edits and absent Captions alike", async () => {
    ...
    const created = await insertCard(client, anecdoteWithImages("Légendée", threeImages));
  ...
  it("keeps order and Captions through an unrelated edit", async () => {
    ...
    const created = await insertCard(client, anecdoteWithImages("Avant", threeImages));
  ...
  it("round-trips removing an Image from the Card", async () => {
    ...
    const created = await insertCard(client, anecdoteWithImages("Allégée", threeImages));
    ...
    const reloaded = await getCard(client, created.id);
    expect(reloaded?.images.map((image) => image.path)).toEqual(["a.webp", "c.webp"]);
  });
});
...
describe("deleteCard", () => {
  ...
  it("removes the row and requests deletion of every referenced object", async () => {
    ...
    const doomed = await insertCard(client, anecdoteWithImages("Illustrée", withImages));

    await deleteCard(client, doomed.id);

    expect(await getCard(client, doomed.id)).toBeNull();
    expect(removals).toEqual([{ bucket: CARD_IMAGES_BUCKET, paths: ["a.webp", "b.webp"] }]);
  });
