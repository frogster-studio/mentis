...
export async function insertCard(client: SupabaseClient, data: CardData): Promise<Card> {
...
export async function listCards(
  ...
  const {
    ...
    count,
    error,
  } = await query.order("updated_at", { ascending: false }).range(offset, offset + pageSize - 1);
  if (error) {
...
export async function getCard(client: SupabaseClient, id: string): Promise<Card | null> {
  const { data: row, error } = await client.from("cards").select("*").eq("id", id).maybeSingle();
  if (error) {
...
export async function deleteCard(client: SupabaseClient, id: string): Promise<void> {
  ...
  const { data: rows, error } = await client.from("cards").delete().eq("id", id).select("images");
  if (error) {
  ...
  const paths = deleted.flatMap((row) => (row.images ?? []).map((image) => image.path));
