...
- `bun run ios` / `bun run android` — compile + launch the development build; `bun run ios:prod` for the production variant. Switching variant needs `bun run prebuild` / `bun run prebuild:prod` first — the native folders still hold the previous variant (they are generated by prebuild and gitignored — never edit `ios/`/`android/` by hand).
- `bun run android:prod` — EAS local build of the production variant (managed keystore), installed on the connected device; Google sign-in matches that keystore's SHA-1, so a debug-signed `run:android --variant release` can never authenticate.
...
- Reads are trusted for `STALE_TIME_MS` (5 min, the client default); a feature's `api.ts` declares its exceptions (`LONG_STALE_TIME_MS` for the catalog and the pseudo, `Infinity` for what issuance fixed, `0` for the store price). Focus and foreground re-read only what is stale; the Player action that moves server state invalidates its keys, never a focus. Tabs mount on their first focus, so a tab's reads wait for its first visit.
