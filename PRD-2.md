...
- Two ways, in this order. **Depuis l'application** (the normal one): open Mentis, press the menu button at the top right of the header, screen « Compte », « Supprimer mon compte », confirm « Supprimer ». **Par email** (the fallback for a Player who no longer has the app): write to `support@frogster-studio.com` **from the Account's own email address** — that address is the identity check, which Google permits — with a ready-made template; answer and deletion within 30 days, in practice a few days.
